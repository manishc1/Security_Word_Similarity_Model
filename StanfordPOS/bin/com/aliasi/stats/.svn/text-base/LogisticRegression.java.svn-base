...
public class LogisticRegression
    ...
    public static LogisticRegression
        ...
        for (int epoch = 0; epoch < maxEpochs; ++epoch) {
            ...
            for (int j = 0; j < numTrainingInstances; ++j) {
                ...
                if (hasPrior) {
                    if (hasSparseInputs) {
                        adjustWeightsWithPrior(weightVectors, xsJ.nonZeroDimensions(),
                                               j, prior, learningRate, numTrainingInstances, lastRegularizations);
                    } else {
                        adjustWeightsWithPriorDense(weightVectors,prior,learningRate,numTrainingInstances);
                    }
                }
            ...
            // catch up feature regularizations before recomputing estimates
            if (hasPrior) {
                if (hasSparseInputs) {
                    adjustWeightsWithPriorAll(weightVectors, prior, learningRate, 
                                              numTrainingInstances, lastRegularizations);
                } else {
                    adjustWeightsWithPriorDense(weightVectors,prior,learningRate,numTrainingInstances);
                }
            }
    ...
    private static void adjustWeightsWithPrior(DenseVector[] weightVectors,
        ...
        double learningRateDividedByNumTrainingInstances
            = learningRate / numTrainingInstances;
        for (DenseVector weightVectorsK : weightVectors) {
            for (int i = 0; i < dimensions.length; ++i) {
                int dim = dimensions[i];
                // cut and paste (version 1)
                ...
                double delta = skippedDimMultiplier * priorGradient * learningRateDividedByNumTrainingInstances;
    ...
    private static void adjustWeightsWithPriorAll(DenseVector[] weightVectors,
                                                  RegressionPrior prior,
                                                  double learningRate,
                                                  int numTrainingInstances,
                                                  int[] lastRegularizations) {
        ...
        for (DenseVector weightVectorsK : weightVectors) {
            for (int dim = 0; dim < numDimensions; ++dim) {
                // cut and paste (version 2) [numTrainingInstances = curInstance]
                double weightVectorsKDim = weightVectorsK.value(dim);
                if (weightVectorsKDim == 0.0)
                    continue;
        ...
        // lastRegularizations reset to zeros after this in main loop
    }

    private static void adjustWeightsWithPriorDense(DenseVector[] weightVectors,
                                                    RegressionPrior prior,
                                                    double learningRate,
                                                    int numTrainingInstances) {
        double learningRateDividedByNumTrainingInstances
