...
public class KMeansClusterer<E> implements Clusterer<E> {
     ...
     * in the clustering or if an element belongs to more than
     * one cluster.
     * @deprecated Use {@link #recluster(Set,Set,Reporter)}
    ...
    private Set<Set<E>> kMeansEpochs(E[] elements,
        ...
        List<Set<E>> resultList = new ArrayList<Set<E>>(numClusters);
        for (int k = 0; k < numClusters; ++k)
            resultList.add(new HashSet<E>());
        for (int i = 0; i < numElements; ++i)
            resultList.get(closestCenters[i]).add(elements[i]);
        Set<Set<E>> result = new HashSet<Set<E>>();
        for (int k = 0; k < numClusters; ++k)
            if (resultList.get(k).size() > 0)
                result.add(resultList.get(k));
        return result;
    }
