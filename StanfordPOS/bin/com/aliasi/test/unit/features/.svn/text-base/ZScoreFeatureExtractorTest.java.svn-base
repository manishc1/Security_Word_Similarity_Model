...
public class ZScoreFeatureExtractorTest {
    ...
    public void testZeros() throws IOException {
        ...
        ZScoreFeatureExtractor<Integer> zsfe
            = new ZScoreFeatureExtractor<Integer>(mfe, corpus);
    ...
    static class MyCorpus extends Corpus<ClassificationHandler<Integer, Classification>> {
        final Integer[] mItems;
        MyCorpus(Integer... items) {
            mItems = items;
        }
        public void visitTrain(ClassificationHandler<Integer,Classification> handler) {
            for (Integer item : mItems)
                handler.handle(item,null);
        }
    }
