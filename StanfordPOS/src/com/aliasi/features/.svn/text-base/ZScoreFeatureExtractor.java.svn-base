...
public class ZScoreFeatureExtractor<E> 
    ...
    /**
     * Return the z-score of the value as determined by the mean
     * and deviation of the specified feature in the training
     * corpus.
     *
     *
     * @return The z-score normalized feature vector.
     * @deprecated Use {@link #zScore(String,double)} instead; this
     * method no longer overrides the the method of the same name in
     * {@link ModifiedFeatureExtractor} because this class no longer
     * overrides {@code ModifiedFeatureExtractor}.
     */
    @Deprecated
    public Number filter(String feature, Number value) {
        return zScore(feature,value.doubleValue());
    }
    ...
    */
    ...
    static <F> Map<String,MeanDev> meanDevs(Corpus<ClassificationHandler<F,Classification>> corpus,
                                            final FeatureExtractor<? super F> extractor) 
        throws IOException {
        
        final Set<String> collectedFeatures = new HashSet<String>();
        corpus.visitTrain(new ClassificationHandler<F,Classification>() {
                            public void handle(F in, Classification c) {
                                collectedFeatures.addAll(extractor.features(in).keySet());
                            }
                          });
