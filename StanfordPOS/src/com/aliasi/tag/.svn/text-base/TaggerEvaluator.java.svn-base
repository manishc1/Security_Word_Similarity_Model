 ...
 * classifier evaluator by {@link #tokenEvaluation()}.  The cases here
...
public class TaggerEvaluator<E> implements ObjectHandler<Tagging<E>> {
    ...
    ClassifierEvaluator<E,Classification> eval(Set<E> knownTokenSet) {
        String[] tags = tags().toArray(Strings.EMPTY_STRING_ARRAY);
        ClassifierEvaluator<E,Classification> evaluator
            = new ClassifierEvaluator<E,Classification>(null,tags,storeTokens());
        for (int i = 0; i < mReferenceTagss.size(); ++i) {
