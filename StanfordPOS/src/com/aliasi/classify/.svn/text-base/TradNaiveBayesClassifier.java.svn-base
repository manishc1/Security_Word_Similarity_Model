 ...
 * Classifier<CharSequence,JointClassification>}.  The compiled
 ...
 * java.util.concurrent.locks.ReadWriteLock}.  The write methods include
 * {@link #handle(CharSequence,Classification)}, {@link
...
public class TradNaiveBayesClassifier
     ...
     * #trainConditional(CharSequence,ConditionalClassification,double,double)}.
     *
     * <p>Implementation for the {@link ClassificationHandler}
     * interface.
     *
    ...
    public static TradNaiveBayesClassifier
        ...
        Iterator<TradNaiveBayesClassifier> it
            = em(initialClassifier,classifierFactory,labeledData,unlabeledData,minTokenCount);
    ...
    static double dataProb(TradNaiveBayesClassifier classifier,
                           Corpus<ClassificationHandler<CharSequence,Classification>> labeledData,
                           Corpus<TextHandler> unlabeledData) throws IOException {

        CaseProbAccumulator accum = new CaseProbAccumulator(classifier);
        labeledData.visitTrain(accum);
    ...
    static class CaseProbAccumulator implements TextHandler, ClassificationHandler<CharSequence,Classification> {
        ...
        public void handle(CharSequence cSeq, Classification cl) {
            mCaseProb += mClassifier.log2CaseProb(cSeq);
        }
        public void handle(char[] cs, int start, int length) {
            handle(new String(cs,start,length),null);
        }
    }
    ...
    static class EmIterator extends Iterators.Buffered<TradNaiveBayesClassifier> {
        ...
        private final Corpus<ClassificationHandler<CharSequence,Classification>> mLabeledData;
        private final Corpus<TextHandler> mUnlabeledData;
        private final double mMinTokenCount;
        private Classifier<CharSequence,JointClassification> mLastClassifier;
        ...
        void trainSup(Corpus<ClassificationHandler<CharSequence,Classification>> labeledData,
        ...
        void trainUnsup(final Corpus<TextHandler> unlabeledData,
                        final TradNaiveBayesClassifier classifier) {
            try {
                unlabeledData.visitTrain(new TextHandler() {
                        public void handle(char[] cs, int start, int length) {
                            CharSequence cSeq = new String(cs,start,length);
        ...
        void compile(TradNaiveBayesClassifier classifier) {
            try {
                ...
                Classifier<CharSequence,JointClassification> lastClassifier
