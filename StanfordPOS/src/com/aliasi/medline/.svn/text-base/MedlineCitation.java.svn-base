 ...
 * record in the 2009 MEDLINE database for the citation of an individual
 * article.
 *
...
public class MedlineCitation {
     ...
     * <blockquote>
     * <a href="http://www.nlm.nih.gov/bsd/licensee/greek_characters.html"
    ...
    static MedlineCitation parse(InputSource inSource,
                                        XMLReader xmlReader)
    ...
    static class Handler extends DelegateHandler {
        ...
        private final List<Chemical> mChemicalList = new ArrayList<Chemical>();
        private final Chemical.Handler mChemicalHandler;
        private final List<String> mCitationSubsetList = new ArrayList<String>();
        private final TextAccumulatorHandler mCitationSubsetHandler
            = new TextAccumulatorHandler();
        private final List<CommentOrCorrection> mCommentOrCorrectionList = new ArrayList<CommentOrCorrection>();
        private final CommentOrCorrection.Handler mCommentsCorrectionsHandler;
        private final List<String> mGeneSymbolList = new ArrayList<String>();
        ...
        private final TextAccumulatorHandler mNumberOfReferencesHandler
            = new TextAccumulatorHandler();
        private final List<PersonalNameSubject> mPersonalNameSubjectList = new ArrayList<PersonalNameSubject>();
        private final PersonalNameSubject.Handler mPersonalNameSubjectHandler;
        private final List<OtherID> mOtherIDList = new ArrayList<OtherID>();
        private final OtherID.Handler mOtherIDHandler;
        private final List<OtherAbstract> mOtherAbstractList = new ArrayList<OtherAbstract>();
        private final OtherAbstract.Handler mOtherAbstractHandler;
        private final List<KeywordList> mKeywordListList = new ArrayList<KeywordList>();
        private final KeywordList.Handler mKeywordListHandler;
        private final List<String> mSpaceFlightMissionList = new ArrayList<String>();
        private final TextAccumulatorHandler mSpaceFlightMissionHandler
            = new TextAccumulatorHandler();
        private final List<Investigator> mInvestigatorList = new ArrayList<Investigator>();
        private final Investigator.Handler mInvestigatorHandler;
        private final List<GeneralNote> mGeneralNoteList = new ArrayList<GeneralNote>();
        ...
        public Handler(DelegatingHandler delegator) {
            ...
            mCommentsCorrectionsHandler 
                = new CommentOrCorrection.Handler(delegator);
            setDelegate(MedlineCitationSet.COMMENTS_CORRECTIONS_ELT,
                        mCommentsCorrectionsHandler);
            
            // for (int i = 0;
            //   i < MedlineCitationSet.COMMENT_OR_CORRECTIONS.length; ++i) {
            //   String elt = MedlineCitationSet.COMMENT_OR_CORRECTIONS[i];
            //   setDelegate(elt,new CommentOrCorrection.Handler(elt,delegator));
            // }
        ...
        public void finishDelegate(String qName, DefaultHandler handler) {
            if (qName.equals(MedlineCitationSet.CITATION_SUBSET_ELT)) {
                ...
                mOtherAbstractList.add(mOtherAbstractHandler
                                       .getOtherAbstract());
            } else if (MedlineCitationSet.COMMENTS_CORRECTIONS_ELT.equals(qName)) {
        ...
        public MedlineCitation getCitation() {
            String[] citationSubsets
                = mCitationSubsetList.toArray(Strings.EMPTY_STRING_ARRAY);
            Chemical[] chemicals
                = mChemicalList.toArray(EMPTY_CHEMICAL_ARRAY);
            KeywordList[] keywordLists
                = mKeywordListList.toArray(EMPTY_KEYWORD_LIST_ARRAY);
            GeneralNote[] generalNotes
                = mGeneralNoteList.toArray(EMPTY_GENERAL_NOTES_ARRAY);
            String[] geneSymbols
                = mGeneSymbolList.toArray(Strings.EMPTY_STRING_ARRAY);
            String[] spaceFlightMissions
                = mSpaceFlightMissionList.toArray(Strings.EMPTY_STRING_ARRAY);
            OtherID[] otherIDs
                = mOtherIDList.toArray(EMPTY_OTHER_ID_ARRAY);
            Investigator[] investigators
                = mInvestigatorList.toArray(EMPTY_INVESTIGATOR_ARRAY);
            OtherAbstract[] otherAbstracts
                = mOtherAbstractList.toArray(EMPTY_OTHER_ABSTRACT_ARRAY);
            CommentOrCorrection[] commentOrCorrections
                = mCommentOrCorrectionList.toArray(EMPTY_COMMENT_OR_CORRECTION_ARRAY);
            PersonalNameSubject[] personalNameSubjects
                = mPersonalNameSubjectList.toArray(EMPTY_PERSONAL_NAME_SUBJECT_ARRAY);
    ...
    static final Chemical[] EMPTY_CHEMICAL_ARRAY
        = new Chemical[0];
    static final KeywordList[] EMPTY_KEYWORD_LIST_ARRAY
        = new KeywordList[0];
    static final GeneralNote[] EMPTY_GENERAL_NOTES_ARRAY
        = new GeneralNote[0];
    static final OtherID[] EMPTY_OTHER_ID_ARRAY
        = new OtherID[0];
    static final Investigator[] EMPTY_INVESTIGATOR_ARRAY
        = new Investigator[0];
    static final OtherAbstract[] EMPTY_OTHER_ABSTRACT_ARRAY
        = new OtherAbstract[0];
    static final CommentOrCorrection[] EMPTY_COMMENT_OR_CORRECTION_ARRAY
        = new CommentOrCorrection[0];
    static final PersonalNameSubject[] EMPTY_PERSONAL_NAME_SUBJECT_ARRAY
        = new PersonalNameSubject[0];
