...
public class WordShapeClassifier {
   ...
   * @param inStr String to calculate word shpape of
   * @param wordShaper Constant for which shaping formula to use
   * @param knownLCWords A set of known lowercase words, which some shapers use
   *           to decide the class of sentence-initial capitalized words.
   *           <i>Warning: This set will be stored in a static variable and
   *           overwrite any previously set list of knownLCWords.</i>
   * @return The wordshape String
   */
   ...
   * @param inStr String to calculate word shpape of
   * @param wordShaper Constant for which shaping formula to use
   * @param markKnownLC Whether to mark known lower case words that appear
   *           capitalized
   * @param knownLCWords A set of known lowercase words, which some shapers use
   *           to decide the class of sentence-initial capitalized words.
   *           <i>Warning: This set will be stored in a static variable and
   ...
   * @return The wordshape String
   */
  private static String wordShape(String inStr, int wordShaper, boolean markKnownLC, Set<String> knownLCWords) {
   ...
   * @param markKnownLC Whether to tag with k suffix a word that is known
   *                    in a list when lowercased
  ...
  public static String wordShapeChris2(String s, boolean markKnownLC, boolean omitIfInBoundary) {
    ...
    if (len <= BOUNDARY_SIZE * 2) {
      return wordShapeChris2Short(s, markKnownLC, len);
    } else {
      return wordShapeChris2Long(s, markKnownLC, omitIfInBoundary, len);
    }
  }

  // Do the simple case of words <= BOUNDARY_SIZE * 2 (i.e., 4) with only 1 object allocation!
  private static String wordShapeChris2Short(String s, boolean markKnownLC, int len) {
    int sbLen = markKnownLC ? len + 1: len;  // markKnownLC makes String 1 longer
   ...
   *
   * <i>Implementation note: This hasn't been optimized like wordShapeChris2,
   * but could be.</i>
   *
   * @param s The String to find the word shape of
   * @param markKnownLC Whether to tag with k suffix a word that is known
  ...
  private static Set<String> knownLCWords = new HashSet<String>();
  ...
  public static String wordShapeDan2Bio(String s, boolean useKnownLC) {
    if (containsGreekLetter(s)) {
      return wordShapeDan2(s, useKnownLC) + "-GREEK";
    } else {
      return wordShapeDan2(s, useKnownLC);
    }
  }
